...
public class LSL {
    ...
    static {
        ...
        switch (Platform.getOSType()) {
            case Platform.WINDOWS:
                inst = Native.load((Platform.is64Bit() ? "liblsl64.dll" : "liblsl32.dll"), dll.class);
                break;
            case Platform.MAC:
                inst = Native.load((Platform.is64Bit() ? "liblsl64.dylib" : "liblsl32.dylib"), dll.class);
                break;
            case Platform.ANDROID:
                // For JNA <= 5.1.0
                System.setProperty("jna.nosys", "false");
                inst = Native.load("lsl", dll.class);
                break;
            default:
                inst = Native.load((Platform.is64Bit() ? "liblsl64.so" : "liblsl32.so"), dll.class);
                if (inst == null)
                    inst = Native.load("liblsl.so", dll.class);
                break;
        }
    }
    ...
    public static class StreamInlet {
        ...
        public int samples_available() {
            return inst.lsl_samples_available(obj);
        }
        ...
        public boolean was_clock_reset() {
            return inst.lsl_was_clock_reset(obj) != 0;
        }
    }
    ...
    public static class ContinuousResolver {
        private final Pointer obj; // the underlying native handle
